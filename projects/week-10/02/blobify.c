...
bool blb_block_get(blb_block_t *block, int32_t offset, uint8_t *value) {
  if ((!block) || (!block->base) || (!value) || (offset < 0) ||
      (offset >= block->size)) {
    return false;
  }
...
bool blb_block_write(blb_block_t *block, uint8_t *data, uint32_t size,
                     int32_t offset) {
  if (!block || !block->base || !data || size == 0 || offset < 0 ||
      (offset + size > block->size))
    return false;

  for (int32_t i = 0; i < size; i++)
    blb_block_put(block, offset + i, data[i]);
  return true;
}
...
bool blb_block_read(blb_block_t *block, uint8_t *out, uint32_t size,
  ...
  if (!block || !block->base || !out || size == 0 || offset < 0 ||
      (offset + size > block->size))
    return false;

  for (int32_t i = 0; i < size; i++)
    blb_block_get(block, offset + i, &out[i]);
  return true;
}
...
int main(void) {
  ...
  blb_blob_toggle_cursor_pin(blob);

  blb_blob_step(blob, 1);
  blb_blob_block_put(blob, 0xAA);

  blb_blob_step(blob, 1);
  blb_blob_block_put(blob, 0xBB);

  blb_blob_jump(blob, 5);
  blb_blob_block_put(blob, 0xCC);

  blb_blob_jump(blob, 7);
  blb_blob_block_put(blob, 0xFF);

  if (!blb_blob_jump(blob, 8))
    printf("jump outside range\n");

  uint8_t value = 0;

  if (blb_blob_block_get(blob, &value))
    printf("read one byte (index 7): %02X\n", value);

  uint8_t data[3] = {0x11, 0x22, 0x33};

  if (blb_blob_block_write(blob, data, 3, 2))
    printf("write 3 bytes (index 2)\n");

  uint8_t out[3] = {0};

  if (blb_blob_block_read(blob, out, 3, 2))
    printf("read 3 bytes (index 2): %02X %02X %02X\n", out[0], out[1], out[2]);

  if (!blb_blob_block_write(blob, data, 3, 6))
    printf("write outside range\n");

  if (!blb_blob_block_read(blob, out, 3, 6))
    printf("read outside range\n");

  printf("\nblock content:\n");
  blb_block_print(blob->block, stdout);
